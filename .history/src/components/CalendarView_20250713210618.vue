...
const loadPeriods = async () => {
  ...
  try {
    ...
    const mostRecentStart = new Date(periods[0]?.startDate)
    ...
    periods.forEach((period, i) => {
      ...
      // Comparer à la date de la période la plus récente
      const isOlderThanMostRecent = startDate < mostRecentStart
      const style = { opacity: isOlderThanMostRecent ? 0.3 : 1 }
