<template>
  <div class="min-h-screen bg-gradient-to-b from-pink-50 to-gray-100 py-8 px-4">
    <div class="max-w-screen-md mx-auto bg-white shadow-xl rounded-2xl p-6 space-y-8">
      ...
      <section
        ...
        class="p-4 bg-yellow-50 border-yellow-400 rounded-md text-yellow-800 text-sm"
      >
  ...
  <div v-if="averageCycleDuration" class="bg-blue-50 rounded-lg shadow-md p-4 border-l-4 border-blue-400">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-xs text-blue-600">
          Calculée à partir de vos {{ periods.length > 1 ? periods.length - 1 : 0 }} derniers cycles
        </p>
      </div>
...
// Surveiller les changements de cycle
watch(userHasChangedCycle, (newValue) => {
  if (newValue) {
    showCycleWarning.value = true

    // Masquer après 5 secondes
    setTimeout(() => {
      showCycleWarning.value = false
      userHasChangedCycle.value = false
    }, 5000)
  }
})
