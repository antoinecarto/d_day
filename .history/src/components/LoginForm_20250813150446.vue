    ...
    <div class="max-w-md w-full space-y-8">
      <div>
        ...
        <p class="mt-2 text-center text-sm text-gray-600">
          <span v-if="isExistingUser">Bon retour ! Vous êtes connectée à votre compte.</span>
          <span v-else>Choisissez comment utiliser l'application</span>
        </p>
      </div>

      <!-- Options de stockage -->
      <div class="bg-white shadow rounded-lg p-6 space-y-6">
        <!-- Message pour utilisatrice existante -->
        <div v-if="isExistingUser" class="bg-green-50 border border-green-200 rounded-lg p-4">
          <div class="flex items-center">
            <span class="text-green-600 text-lg mr-2">✅</span>
            <div>
              <h3 class="font-medium text-green-800">Compte existant détecté</h3>
        ...
        <!-- Options de stockage (cachées si utilisatrice existante) -->
          ...
          >
            <div class="flex items-start space-x-3">
              ...
              <div class="flex-1">
                ...
                <div class="mt-2 text-xs text-gray-500">
                  ✓ Synchronisation multi-appareils • ✓ Sauvegarde automatique • ⚠️ Nécessite
                  internet
                </div>
              </div>
            </div>
        ...
        <!-- Formulaire de connexion (affiché seulement si cloud sélectionné ET pas utilisatrice existante) -->
        <div
          v-if="selectedOption === 'firebase' && !isExistingUser"
        ...
        <div class="pt-4">
          ...
          >
            <span v-if="isExistingUser"> ☁️ Accéder à mes données </span>
            <span v-else-if="selectedOption === 'local'"> 💾 Commencer en local </span>
            <span v-else-if="isConnected"> ☁️ Accéder à mes données </span>
            <span v-else> Se connecter d'abord </span>
          </button>
        </div>

        <!-- Info supplémentaires (cachées si utilisatrice existante) -->
        <div v-if="!isExistingUser" class="text-xs text-gray-500 space-y-1 pt-4 border-t">
...
const isExistingUser = ref(false) // Nouvelle variable pour détecter utilisatrice existante
const router = useRouter()

// État calculé
const canStart = computed(() => {
  if (isExistingUser.value) return true // Utilisatrice existante peut toujours démarrer
...
const startApp = async () => {
  ...
  if (isExistingUser.value || (selectedOption.value === 'firebase' && isConnected.value)) {
    storageType = 'firebase'
  }

  // Configurer le type de stockage
  storageService.setStorageType(storageType)
...
onMounted(() => {
  ...
  onAuthStateChanged(auth, (user) => {
    ...
    // Détecter si c'est une utilisatrice existante
    if (user?.uid) {
      isExistingUser.value = true
      selectedOption.value = 'firebase' // Forcer le mode Firebase
      console.log('Utilisatrice Firebase existante détectée:', user.email)
    } else {
